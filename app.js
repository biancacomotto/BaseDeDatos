...
app.get('/pelicula/:id', (req, res) => {
    ...
    // Consulta SQL para obtener los datos de la película, elenco y crew (lo que nos interesa de los datos)
    const query = `
    SELECT DISTINCT
      movie.*,
  ...
  `;
    ...
    db.all(query, [movieId], (err, rows) => {
        if (err) {
            ...
            // Organizamos los datos en un objeto de película con elenco y crew
            const movieData = {
                id: rows[0].id,
                title: rows[0].title,
            ...
            // Creamos un objeto para almacenar los escritores
            rows.forEach((row) => {
                if (row.crew_member_id && row.crew_member_name && row.department_name && row.job) {
                    ...
                    if (!isDuplicate) {
                        // Si no existe, agregamos los datos de los escritores
...
app.get('/person/:id', (req, res) => {
    ...
    // Ejecutamos las consulta para películas en las que actuó
    db.all(actedQuery, [personId], (err, actedRows) => {
        if (!err) {
            results.moviesActed = actedRows;

            // Ejecutamos la consulta para películas dirigidas
            db.all(directedQuery, [personId], (err, directedRows) => {
                if (!err) {
                    ...
                    db.get(actorQuery, [personId], (err, actorRow) => {
                        if (!err) {
                            results.actorName = actorRow.person_name; // Nombre del actor
                        }
